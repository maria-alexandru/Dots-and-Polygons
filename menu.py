...
def create_buttons(screen_width, screen_height):
    ...
    theme_button = Button(
        image=polygon_image,
        pos=(screen_width // 2, title_button_spacing + int(470 * scale_factor)),
        text_input=f"Theme: {theme_options[theme_id]}",
        font=font,
        base_color="White",
        hovering_color="#d7fcd4",
    )
    return [polygon_button, grid_button, opponent_button, music_button, play_button, quit_button, theme_button]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if buttons[5].checkForInput(MENU_MOUSE_POS):  # Quit button
                    ...
                    pygame.quit()
                    sys.exit()
                if buttons[6].checkForInput(MENU_MOUSE_POS):  # Theme button
                    button_sound.play()
